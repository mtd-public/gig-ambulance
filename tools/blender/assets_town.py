...
from assets_props import H, bench_at, bush_at, lamp_at, pine_at, round_tree_at
from kit import FACING, Model, arch_pts, at, ball, box, cone, cyl, dome, prism, torus

TILE = 8.0
HALF = TILE / 2
ROAD = 2.5          # half road width
SIDE = HALF - ROAD  # pavement width (1.5)
...
def paint(m, mat_, d, a0, a1, lat, width):
    """Road marking along direction d from `along` a0..a1 at lateral offset lat."""
    dx, dy = DIRS[d]
    px, py = -dy, dx
    a = (a0 + a1) / 2
    size = (width, a1 - a0) if dx == 0 else (a1 - a0, width)
    m.add(box(size[0], size[1], 0.02), mat_, (dx * a + px * lat, dy * a + py * lat, 0.01))


def road_tile(name, conns, desc, crosswalk=False, junction_zebras=False):
    m = Model(name, "streets", desc, tile=[1, 1], connections=conns)
    m.add(box(TILE, TILE, BASE), "road", (0, 0, -BASE / 2))
    # pavements: 4 corner squares + a strip on every closed side
    for sx in (-1, 1):
        for sy in (-1, 1):
            xs = sorted((sx * ROAD, sx * HALF))
            ys = sorted((sy * ROAD, sy * HALF))
            slab(m, "sidewalk", *xs, *ys, H, 0.04)
    for d, (dx, dy) in DIRS.items():
        if d in conns:
            continue
        if dx == 0:
            slab(m, "sidewalk", -ROAD, ROAD, *sorted((dy * ROAD, dy * HALF)), H, 0.04)
        else:
            slab(m, "sidewalk", *sorted((dx * ROAD, dx * HALF)), -ROAD, ROAD, H, 0.04)
    # yellow edge lines (inset 0.3 from the kerb), continuous across tiles
    e, w = ROAD - 0.3, 0.14
    for d in DIRS:
        if d in conns:
            for lat in (-e, e):
            ...
            # closed side: line runs perpendicular to d at along = e
            px, py = -dy, dx
            cx, cy = dx * e, dy * e
            L = hi - lo
            mid = (lo + hi) / 2
            size = (L, w) if dx == 0 else (w, L)
            m.add(box(size[0], size[1], 0.02), "roadline", (cx + (mid if dx == 0 else 0),
                                                              cy + (mid if dx != 0 else 0), 0.01))
    # corner joins where two adjacent arms are open
    ...
    # white centre dashes, period 2 m so they line up tile-to-tile
    straight = conns in ("NS", "EW")
    for d in conns:
        if junction_zebras:
            break
        centres = (1.0, 3.0) if straight or len(conns) == 2 else (3.0,)
        for c in centres:
            if crosswalk and c == 1.0:
                continue
            paint(m, "zebra", d, c - 0.5, c + 0.5, 0, 0.16)
    if crosswalk:  # zebra across the middle of a straight N-S road
        for i in range(6):
            m.add(box(0.45, 1.8, 0.02), "zebra", (-2.0 + i * 0.8, 0, 0.012))
    if junction_zebras:
        for d in conns:
            dx, dy = DIRS[d]
            for i in range(6):
                lat = -2.0 + i * 0.8
                size = (0.45, 1.2) if dx == 0 else (1.2, 0.45)
                m.add(box(size[0], size[1], 0.02), "zebra", (dx * 3.2 - dy * lat, dy * 3.2 + dx * lat, 0.012))
    return m


def road_straight():
    return road_tile("road_straight", "NS", "Straight road, N-S.")


def road_crosswalk():
    return road_tile("road_crosswalk", "NS", "Straight road with zebra crossing (pedestrians spawn here).",
                     crosswalk=True)


def road_corner():
    return road_tile("road_corner", "EN", "90 deg corner, N-E. Prime drift spot.")
...
def lot_base(m, tiles=1, mat_="grass", path=True, path_w=1.6):
    w = TILE * tiles
    slab(m, mat_, -w / 2, w / 2, -HALF, HALF, H, 0.06)
    if path:
        m.add(box(path_w, 2.2, 0.03), "sidewalk", (0, -HALF + 1.1, H + 0.005))
...
def cottage(name, desc, wall, roof, door_col, chimney=True):
    ...
    for x in (-2.2, 2.2):  # low picket at the front
        m.add(box(2.6, 0.12, 0.5, 0.04), "trim_green", (x, -3.7, H + 0.25))
    return m
...
def apartment():
    ...
    lot_base(m, mat_="sidewalk", path=False)
...
def hospital():
    ...
    m.add(box(8.0, 3.2, 0.03), "sidewalk", (0, -2.4, H + 0.005))           # drive-in apron
...
def park():
    ...
    for x, y, s, t in ((-2.8, 2.8, 1.0, 0), (2.9, 2.6, 0.8, 20), (-2.7, -2.9, 0.75, 40), (2.8, -2.8, 0.95, 60)):
        pine_at(m, x, y, H, s, t)
    bench_at(m, -2.0, 1.4, H)
    lamp_at(m, 1.3, 1.3, H)
    bush_at(m, 1.6, -1.6, H, 0.7)
    return m
...
ALL = [road_straight, road_crosswalk, road_corner, road_t, road_cross, road_end, lot_grass, park,
       hospital, house_lilac, house_peach, apartment, cafe, pharmacy, office, lighthouse]
